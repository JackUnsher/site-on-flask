...
## Рабочая конфигурация amvera.yml

Ниже приведена проверенная рабочая конфигурация `amvera.yml`, которая успешно запускает сборку на Amvera:
...
**Важные замечания:**
- Не используйте секции `env` или `variables` и `postDeploy` в этой конфигурации
- Для постоянного хранения данных используйте `persistenceMount: /data`
- Переменные окружения настраивайте через файл `.env` в корне проекта или через интерфейс Amvera в разделе "Переменные"
...
### Через интерфейс Amvera

Альтернативно, можно настроить переменные окружения через интерфейс Amvera в разделе "Настройки" -> "Переменные окружения". Добавьте те же самые переменные, что указаны выше.

**Важно:** Не рекомендуется использовать секцию `build.env` в файле `amvera.yml`, так как это может вызвать проблемы при развертывании. 